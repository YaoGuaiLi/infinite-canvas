...
apiErrors: { requestFailed: "请求失败", requestCanceled: "请求已取消", baseUrlRequired: "请先配置 Base URL", apiKeyRequired: "请先配置 API Key", authenticationFailed: "鉴权失败，请检查 API Key、套餐权限或模型权限", rateLimited: "请求被限流或额度不足，请稍后重试", notFound: "接口地址不存在（404），请检查 Base URL 和模型选择", badGateway: "网关错误（502），接口服务暂时不可用，请稍后重试", serviceBusy: "服务繁忙（503），请稍后重试", httpFailed: "请求失败（HTTP {{status}}），请检查 Base URL 和 API Key 是否正确", htmlError: "服务返回了 HTML 错误页面（{{preview}}）", imageTimeout: "生图请求超时，接口长时间没有返回，请稍后重试或更换渠道", audioModelRequired: "请先配置音频模型", audioGenerationFailed: "音频生成失败", scriptNoAudio: "模型调用脚本没有返回音频", geminiAudioUnsupported: "Gemini 调用格式暂不支持音频生成，请使用 OpenAI 格式渠道", invalidImageSizeFormat: "图像尺寸格式不支持，请使用 auto、9:16 或 1024x1024", positiveImageRatio: "图像比例必须是正数，例如 9:16", imageRatioLimit: "图像宽高比不能超过 3:1，请调整尺寸", positiveImageDimensions: "图像尺寸必须是正整数，例如 1024x1024", imageDimensionStep: "图像尺寸的宽高必须是 16 的倍数，请调整尺寸", imageEdgeLimit: "图像尺寸最长边不能超过 3840px，请调整尺寸", imagePixelLimit: "图像总像素需在 655360 到 8294400 之间，请调整尺寸", unknownImageResponse: "接口返回了未知格式的数据（字段：{{fields}}），请检查模型或接口兼容性", noImageReturned: "接口没有返回图片，请检查提示词是否触发安全审核或模型是否支持该操作", geminiRejected: "Gemini 拒绝了本次请求：{{reason}}", geminiNoImage: "Gemini 接口没有返回图片", maskModelUnsupported: "蒙版编辑暂不支持该模型，请使用其他渠道", noContent: "没有返回内容", modelReadFailed: "读取模型失败", videoTimeout: "{{provider}}视频生成超时，请稍后重试", pluginVideoExpired: "插件视频任务已失效，请重新生成", scriptNoVideo: "模型调用脚本没有返回视频", noPlayableVideo: "视频接口没有返回可播放的视频", noVideoTaskId: "视频接口没有返回任务 ID", videoTaskCreateFailed: "视频任务创建失败", videoGenerationFailed: "视频生成失败", videoTaskQueryFailed: "视频任务查询失败", videoPromptRequired: "请输入视频提示词，或连接参考图片/视频/音频", referenceImageReadFailed: "参考图读取失败，请换一张图片或重新上传", invalidReferenceVideo: "参考视频必须是公网 URL、资产 ID，或本地已保存的视频", invalidReferenceAudio: "参考音频必须是公网 URL、资产 ID，或本地已保存的音频", videoModelRequired: "请先配置视频模型", geminiVideoUnsupported: "Gemini 调用格式暂不支持视频生成，请使用 OpenAI 格式渠道", noVideoTask: "接口没有返回视频任务", videoDownloadFailed: "视频下载失败", localAssetReadFailed: "读取本地资产失败" , apimartMaskUnsupported: "APIMart 渠道暂不支持蒙版编辑", apimartUploadFailed: "APIMart 参考图上传失败：{{detail}}", apimartUploadNoUrl: "APIMart 参考图上传后未返回 URL", apimartReferenceRequired: "该模型需要提示词或参考图/参考视频", apimartTooManyReferences: "APIMart 图片数量最多 9 张", apimartTaskQueryFailed: "APIMart 任务查询失败", apimartTaskFailed: "APIMart 生成任务失败", apimartTaskTimeout: "APIMart 生成任务超时，请稍后重试", apimartNoTaskId: "APIMart 接口没有返回任务 ID" },
    ...
    navigation: {
        ...
        capabilities: { title: "本地能力直调（MCP 工具）", description: "连接后可直接从网页调用本地 Agent 注册的任意 MCP 工具，包含画布操作、时间轴素材导入、GitHub 备份、APIMart 视频与 WebGPU 深度提取。", inputLabel: "入参 JSON", run: "执行", invalidJson: "入参不是合法 JSON" },
    workflows: {
        badge: "预设验证模板",
        title: "示例工作流",
        description: "精选验证过的画布工作流模板。点击一键将节点与连线拓扑克隆为新的画布工程，开箱即用。",
        useTemplate: "使用此工作流",
        instantiated: "已载入工作流「{{title}}」，已生成专属画布",
        tags: {
            video: "视频流转",
            image: "图像生成",
            audio: "语音对口型",
        ...
        templates: {
            motionTransfer: {
                title: "短视频动作迁移",
                desc: "将选定的人物角色全身照片与特定舞蹈/街舞分镜动作连线融合，输出动作连贯的动作迁移短视频。",
            },
            firstLastFrame: {
                title: "首尾帧平滑过渡视频",
                desc: "连接起始关键帧与结束画面两张图片，在两帧间插值计算自然过渡动作，实现镜头平滑流转。",
            },
            lipSync: {
                title: "人物肖像精准对口型",
                desc: "连接单张人物肖像图片与配音音频，驱动嘴型动作与面部微表情随声音节拍精准同步。",
            },
            storyboardSerial: {
                title: "多机位分镜连拍组",
                desc: "基于同一组风格与尺寸配置，链式并发生成开场、冲突、高潮、结尾 4 个分镜镜头画面。",
            },
        },
    },
    editor: {
        title: "AI 剪辑",
        subtitle: "多轨时间轴 · AI 自动剪辑与配音",
        refresh: "重新加载编辑器",
        fullscreen: "全屏模式",
        exitFullscreen: "退出全屏",
        openExternal: "在新标签页打开",
        importAssets: "导入素材",
        importModalTitle: "导入素材至时间轴剪辑器",
        tabMyAssets: "我的资产",
        tabCanvasNodes: "画布素材",
        selectAssetsHint: "勾选需要送入时间轴的媒体资产",
        selectProject: "选择画布项目",
        selectedCount: "已选 {{count}} 项素材",
        confirmImport: "导入选中的 {{count}} 项素材",
        noMediaAssets: "「我的资产」中暂无可用的图片、视频或音频素材",
        noCanvasMedia: "该画布项目中暂无图片、视频或音频节点素材",
        importEmpty: "请至少勾选一项有效素材",
        importSuccess: "已成功将 {{count}} 项素材传入时间轴剪辑器",
        importFailed: "素材载入失败，请稍后重试",
        editorNotReady: "编辑器尚未加载完成，请稍候",
    },
    topNav: {
        openMenu: "打开导航菜单",
        menu: "导航菜单",
    ...
    agent: {
        ...
        siteTools: { canvasList: "画布列表", generationStatus: "生成任务状态", imageConfig: "生图配置", imageGenerate: "生图工作台生成", videoConfig: "视频配置", videoGenerate: "视频创作台生成", promptSearch: "搜索提示词", assetList: "资产列表", assetAdd: "添加资产", unknownTool: "未知工具：{{name}}", canvasLoading: "画布还在加载中，请稍后重试", canvasHint: "用 site_navigate 跳转 /canvas/{id} 打开对应画布", assetsLoading: "资产还在加载中，请稍后重试", assetTitleRequired: "请提供资产标题 title", textContentRequired: "kind=text 时需要提供 content 文本内容", imageUrlRequired: "kind=image 时需要提供 imageUrl（图片地址或 dataURL）", imageReadFailed: "无法读取该图片地址，请改用 dataURL 或可跨域访问的图片链接", assetKindUnsupported: "assets_add 仅支持 kind=text 或 kind=image", imageGenerationStarted: "已跳转生图工作台并触发生成，可用 generation_get_status 查询任务", imageConfigApplied: "已跳转生图工作台并填入参数，未触发生成", videoGenerationStarted: "已跳转视频创作台并触发生成，可用 generation_get_status 查询任务", videoConfigApplied: "已跳转视频创作台并填入参数，未触发生成", workflowsApplyTemplate: "应用工作流模板", editorImportMedia: "导入素材到剪辑器", syncGithubBackup: "GitHub 备份", apimartGenerateVideo: "APIMart 生成视频", apimartTaskStatus: "APIMart 任务状态", depthModelStatus: "深度模型状态", depthModelPrefetch: "预下载深度模型", unknownWorkflowTemplate: "未知的工作流模板", editorNotOpen: "请先打开 AI 剪辑页面（/editor）再执行素材导入", editorNoMedia: "选定的来源中没有可导入的媒体", editorImportTimeout: "素材导入超时，请检查剪辑器是否正常加载", githubProviderRequired: "请先在配置 → 云同步中切换为 GitHub 仓库模式", githubConfigRequired: "请先在配置中填写 GitHub 仓库和访问令牌", apimartChannelRequired: "没有找到 APIMart 渠道，请先在配置中新建", apimartStarted: "APIMart 视频任务已提交，结果将写入「我的资产」", apimartTaskNotFound: "未找到该 APIMart 任务" },
        connect: { pluginTitle: "方式一：一键唤醒本机 Agent 插件安装（支持 Codex / CodeG / 任意 MCP）", pluginText: "通过下方的原生协议或命令，把本仓库内修改好的 Infinite Canvas 插件（含 timeline-editor 与 creative-studio 双技能）安装到本机的 Agent 运行时中；插件启动画布时会自动带起本地 Agent 并填好连接信息。", pluginTextEnhanced: "通过下方按钮直接唤醒本机已安装的 Codex 或 CodeG 一键安装本仓库增强版插件，或一键复制通用 MCP 启动指令。兼容 Codex、CodeG、ZCode、Cursor、Claude Code、Cline 等所有支持 MCP 协议的 Agent。", directTitle: "方式二：直接运行本地 Agent 源码（支持视频编辑与时间轴联动）", directText: "不使用 Agent 插件时，在终端运行下面命令，再回到网页里连接或手动填入 Local URL 和 Connect token。", directTextEnhanced: "由于本仓库已深度整合 Timeline Studio 时间轴剪辑与 APIMart 视频流转，npm 上的官方旧包尚未包含这些新技能。请通过下方一键克隆运行命令，或下载本仓库源码在本地启动：", downloadAgentSource: "下载本仓库源码（含修改后的完整 canvas-agent）：", ghDirect: "GitHub 直链下载", ghProxy: "GitHub 加速直链下载", repoLink: "查看仓库主页", agentSourceLink: "查看 canvas-agent 源码目录", oneClickCodex: "一键唤醒安装到 Codex", oneClickCodeg: "一键唤醒安装到 CodeG", oneClickZed: "安装到 Zed / VSCode", oneClickMcp: "复制通用 MCP 启动命令", cloneRunTitle: "本仓库最新 Agent 源码一键克隆并启动：", viewPluginDocs: "查看插件与技能文档", agentCompatibilityHint: "兼容说明：支持 Codex、CodeG、ZCode、Cursor、Cline、Claude Code 等任意支持 MCP 规范的 Agent 客户端。", commandCopied: "命令已复制", pluginReminder: "安装提醒", pluginReminderText: "只有把插件或 MCP 真正注册到 Agent 运行时，工具列表才会进入上下文并增加 token 消耗；仅运行本地 Agent 不会自动注册 MCP 工具。", copyCommand: "复制命令", title: "连接本地 Agent", description: "按使用场景选择一种连接方式。", webConnection: "网页连接", autoDiscover: "默认自动读取 Local URL 和 Connect token，失败时再手动填写。", disconnect: "断开", connect: "连接", localAddress: "本地地址", urlPlaceholder: "例如 http://127.0.0.1:17371", token: "连接 Token", tokenPlaceholder: "自动发现，或手动填入 Connect token" },
