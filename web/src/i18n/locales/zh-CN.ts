...
apiErrors: { requestFailed: "请求失败", requestCanceled: "请求已取消", baseUrlRequired: "请先配置 Base URL", apiKeyRequired: "请先配置 API Key", authenticationFailed: "鉴权失败，请检查 API Key、套餐权限或模型权限", rateLimited: "请求被限流或额度不足，请稍后重试", notFound: "接口地址不存在（404），请检查 Base URL 和模型选择", badGateway: "网关错误（502），接口服务暂时不可用，请稍后重试", serviceBusy: "服务繁忙（503），请稍后重试", httpFailed: "请求失败（HTTP {{status}}），请检查 Base URL 和 API Key 是否正确", htmlError: "服务返回了 HTML 错误页面（{{preview}}）", imageTimeout: "生图请求超时，接口长时间没有返回，请稍后重试或更换渠道", audioModelRequired: "请先配置音频模型", audioGenerationFailed: "音频生成失败", scriptNoAudio: "模型调用脚本没有返回音频", geminiAudioUnsupported: "Gemini 调用格式暂不支持音频生成，请使用 OpenAI 格式渠道", invalidImageSizeFormat: "图像尺寸格式不支持，请使用 auto、9:16 或 1024x1024", positiveImageRatio: "图像比例必须是正数，例如 9:16", imageRatioLimit: "图像宽高比不能超过 3:1，请调整尺寸", positiveImageDimensions: "图像尺寸必须是正整数，例如 1024x1024", imageDimensionStep: "图像尺寸的宽高必须是 16 的倍数，请调整尺寸", imageEdgeLimit: "图像尺寸最长边不能超过 3840px，请调整尺寸", imagePixelLimit: "图像总像素需在 655360 到 8294400 之间，请调整尺寸", unknownImageResponse: "接口返回了未知格式的数据（字段：{{fields}}），请检查模型或接口兼容性", noImageReturned: "接口没有返回图片，请检查提示词是否触发安全审核或模型是否支持该操作", geminiRejected: "Gemini 拒绝了本次请求：{{reason}}", geminiNoImage: "Gemini 接口没有返回图片", maskModelUnsupported: "蒙版编辑暂不支持该模型，请使用其他渠道", noContent: "没有返回内容", modelReadFailed: "读取模型失败", videoTimeout: "{{provider}}视频生成超时，请稍后重试", pluginVideoExpired: "插件视频任务已失效，请重新生成", scriptNoVideo: "模型调用脚本没有返回视频", noPlayableVideo: "视频接口没有返回可播放的视频", noVideoTaskId: "视频接口没有返回任务 ID", videoTaskCreateFailed: "视频任务创建失败", videoGenerationFailed: "视频生成失败", videoTaskQueryFailed: "视频任务查询失败", videoPromptRequired: "请输入视频提示词，或连接参考图片/视频/音频", referenceImageReadFailed: "参考图读取失败，请换一张图片或重新上传", invalidReferenceVideo: "参考视频必须是公网 URL、资产 ID，或本地已保存的视频", invalidReferenceAudio: "参考音频必须是公网 URL、资产 ID，或本地已保存的音频", videoModelRequired: "请先配置视频模型", geminiVideoUnsupported: "Gemini 调用格式暂不支持视频生成，请使用 OpenAI 格式渠道", noVideoTask: "接口没有返回视频任务", videoDownloadFailed: "视频下载失败", localAssetReadFailed: "读取本地资产失败" , apimartMaskUnsupported: "APIMart 渠道暂不支持蒙版编辑", apimartUploadFailed: "APIMart 参考图上传失败：{{detail}}", apimartUploadNoUrl: "APIMart 参考图上传后未返回 URL", apimartReferenceRequired: "该模型需要提示词或参考图/参考视频", apimartTooManyReferences: "APIMart 图片数量最多 9 张", apimartTaskQueryFailed: "APIMart 任务查询失败", apimartTaskFailed: "APIMart 生成任务失败", apimartTaskTimeout: "APIMart 生成任务超时，请稍后重试", apimartNoTaskId: "APIMart 接口没有返回任务 ID" },
    ...
    agent: {
        ...
        connect: { pluginTitle: "方式一：一键唤醒本机 Agent 插件安装（不限 Codex，支持任意 MCP Agent）", pluginText: "通过下方的原生协议或命令，把本仓库内修改好的 Infinite Canvas 插件（含 timeline-editor 与 creative-studio 双技能）安装到本机的 Agent 运行时中；插件启动画布时会自动带起本地 Agent 并填好连接信息。", pluginTextEnhanced: "通过下方的原生协议或一键复制 MCP 命令，把本仓库内已增强的 Infinite Canvas 插件（含 timeline-editor 时间轴剪辑与 creative-studio 全流程调度两大技能）安装到本机的 Agent 运行时。兼容 Codex、ZCode、Cursor、Claude Code、Cline 等所有支持 MCP 协议的 Agent。", directTitle: "方式二：直接运行 Agent（本仓库源码）", directText: "不使用 Agent 插件时，在终端运行下面命令，再回到网页里连接或手动填入 Local URL 和 Connect token。", directTextEnhanced: "推荐直接运行 npm 上发布的稳定版本；需要使用仓库源码版（含最新技能与剪辑器联动）时，可通过下方 GitHub 直链或加速直链下载源码压缩包，解压后按 README 说明启动 canvas-agent。", downloadAgentSource: "下载本仓库源码（含 canvas-agent 与插件完整源码）：", ghDirect: "GitHub 直链下载", ghProxy: "GitHub 加速直链下载", repoLink: "查看仓库主页", agentSourceLink: "查看 canvas-agent 源码目录", oneClickCodex: "一键安装到 Codex", oneClickZed: "一键安装到 Zed / VSCode", oneClickMcp: "复制 MCP 启动命令", viewPluginDocs: "查看插件与技能文档", agentCompatibilityHint: "兼容范围：任何支持 MCP（Model Context Protocol）的 Agent 均可安装本插件，例如 Codex、ZCode、Cursor、Cline、Claude Code、Windsurf 等。", commandCopied: "命令已复制", pluginReminder: "安装提醒", pluginReminderText: "只有把插件或 MCP 真正注册到 Agent 运行时，工具列表才会进入上下文并增加 token 消耗；仅运行 npx -y @basketikun/canvas-agent@latest 启动本地 Agent 不会注册 MCP 工具。", copyCommand: "复制命令", title: "连接本地 Agent", description: "按使用场景选择一种连接方式。", webConnection: "网页连接", autoDiscover: "默认自动读取 Local URL 和 Connect token，失败时再手动填写。", disconnect: "断开", connect: "连接", localAddress: "本地地址", urlPlaceholder: "例如 http://127.0.0.1:17371", token: "连接 Token", tokenPlaceholder: "自动发现，或手动填入 Connect token" },
