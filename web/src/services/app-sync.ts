...
import { downloadGithubFile, uploadGithubFile } from "@/services/github-sync";
import { downloadWebdavFile, uploadWebdavFile, WEBDAV_MANIFEST_FILE_NAME } from "@/services/webdav-sync";
import type { Asset } from "@/stores/use-asset-store";
import { useAssetStore } from "@/stores/use-asset-store";
import type { GithubSyncConfig, WebdavSyncConfig } from "@/stores/use-config-store";
...
export function createWebdavTransport(config: WebdavSyncConfig): AppSyncTransport {
    return {
        downloadFile: (path) => downloadWebdavFile(config, path),
        uploadFile: (path, file, contentType) => uploadWebdavFile(config, path, file, contentType),
    };
}

export function createGithubTransport(config: GithubSyncConfig): AppSyncTransport {
    return {
        downloadFile: (path) => downloadGithubFile(config, path),
        uploadFile: (path, file, contentType) => uploadGithubFile(config, path, file, contentType),
    };
}

export async function syncAppDataToCloud(transport: AppSyncTransport, onProgress?: AppSyncProgress): Promise<AppSyncResult> {
...
async function syncDomain<T>(transport: AppSyncTransport, onProgress: AppSyncProgress | undefined, options: SyncDomainOptions<T>): Promise<SyncDomainResult<T>> {
    try {
        ...
        await transport.uploadFile(domainPath(options.key, WEBDAV_MANIFEST_FILE_NAME), manifestFile, "application/json");
...
async function readDomainManifest<T>(transport: AppSyncTransport, domain: DomainKey, emptyData: T): Promise<DomainManifest<T> | null> {
    const file = await transport.downloadFile(domainPath(domain, WEBDAV_MANIFEST_FILE_NAME));
    if (!file) return null;
...
async function downloadMissingFiles<T>(transport: AppSyncTransport, domain: DomainKey, data: T, remoteFiles: AppSyncFile[], onProgress?: AppSyncProgress) {
    ...
    await runWithConcurrency(tasks, FILE_CONCURRENCY, async (remoteFile) => {
        const blob = await transport.downloadFile(remoteFile.path);
        if (!blob) return;
...
async function uploadChangedFiles<T>(transport: AppSyncTransport, domain: DomainKey, data: T, remoteFiles: AppSyncFile[], onProgress?: AppSyncProgress) {
    ...
    await runWithConcurrency(tasks, FILE_CONCURRENCY, async ({ item, blob }) => {
        await transport.uploadFile(item.path, blob, item.mimeType);
        uploadedFiles += 1;
