...
import { defaultGithubSyncConfig, useConfigStore, type AiConfig, type GithubSyncConfig, type SyncProvider, type WebdavSyncConfig } from "@/stores/use-config-store";
...
export function exportAppConfig() {
    const { config, webdav, github, syncProvider } = useConfigStore.getState();
    const { sources, schedule } = usePromptSourceStore.getState();
    const data: AppConfigFile = { app: "infinite-canvas", version: 1, exportedAt: new Date().toISOString(), config, webdav, github, syncProvider, promptSources: { sources, schedule } };
...
export async function importAppConfig(file: File) {
    ...
    useConfigStore.setState({
        config: data.config,
        webdav: data.webdav,
        ...(data.github ? { github: { ...defaultGithubSyncConfig, ...data.github } } : {}),
        ...(data.syncProvider ? { syncProvider: data.syncProvider } : {}),
    });
